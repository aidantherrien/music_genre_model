...
def process_data(data_path, genre_map):
    ...
    # Gather all audio files first to determine total count
    audio_files_info = []
    for genre_folder in os.listdir(data_path):
        ...
        for audio_file in os.listdir(genre_folder_path):
            if audio_file.endswith('.wav') or audio_file.endswith('.mp3'):
                ...
                audio_files_info.append((genre_folder, audio_file, audio_file_path))

    total_files = len(audio_files_info)
    print(f"\nFound {total_files} audio files in '{data_path}' to process.\n")

    for idx, (genre_folder, audio_file, audio_file_path) in enumerate(audio_files_info, start=1):
        genre_label = genre_map[genre_folder]

        # Extract song name (title) 
        filename_no_ext = os.path.splitext(audio_file)[0]
        if " - " in filename_no_ext:
            title = filename_no_ext.split(" - ", 1)[1]
        else:
            title = filename_no_ext

        print(f"[{idx}/{total_files}]: ({genre_folder}) - {title}")

        try:
            mfcc, chroma, spectral_contrast = extract_features(audio_file_path)
        except Exception as e:
            print(f"Error: Skipping file due to error: {audio_file_path}\n   Error: {e}")
            continue
        ...
        if feature_vector.shape[0] != 64:
            print(f"Error: Skipping file due to incorrect feature vector size: {feature_vector.shape[0]} != 64")
            continue
...
def main():
    ...
    reverse_genre_map = {idx: genre for genre, idx in genre_map.items()}
    ...
    genre_names = [reverse_genre_map[label] for label in all_labels]
